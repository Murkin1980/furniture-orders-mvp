...
- Calculator leads сохраняют `calculatorMeta` в `raw_payload`: `calculatorId`, `categoryCode`, `units`, `materialMultiplier`, `estimate`.
...
- Stage 4.02: редактор цен, коэффициентов и формул калькулятора в админке.
- После стабилизации Stage 3/4.01 применить D1 migrations к production и выполнить deploy.
