...
| 3D rendering pipeline | `[########--] 75%` | Render storage paths, guarded upload, read API, admin order visibility, and fail-closed HTTP-to-executor wiring are tested; default node mode remains dry-run | Install an approved Windows SketchUp/render executor |
