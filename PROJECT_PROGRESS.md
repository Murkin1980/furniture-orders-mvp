...
| Production infrastructure | `[##########] 100%` | Cloudflare Pages/R2 are verified; PS.kz VPS incident is isolated, Google Cloud test VPS is online, authenticated Cloudflare -> VPS read-only smoke passes, controlled live HTML deploy succeeded, and public test landing returns 200 | Use the Google test VPS for temporary landing tests; rebuild/replace PS.kz before any production reuse |
...
| 3D rendering pipeline | `[########--] 83%` | Render storage paths, guarded upload, read API, admin visibility, render-ready file-queue response contract, fail-closed HTTP-to-executor wiring, and Ruby outbox finalizer are tested or guarded; default node mode remains dry-run | Build the real local SketchUp/EasyKitchen geometry and render adapter |
