...
| 3D rendering pipeline | `[########--] 80%` | Render storage paths, guarded upload, read API, admin visibility, render-ready file-queue response contract, and fail-closed HTTP-to-executor wiring are tested; default node mode remains dry-run | Build/review the local SketchUp Ruby consumer or approved render executor |
