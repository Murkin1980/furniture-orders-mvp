...
| 3D rendering pipeline | `[#######---] 70%` | Render storage paths, guarded upload, read API, and fail-closed HTTP-to-executor wiring are tested; default node mode remains dry-run | Surface render artifacts in admin order view, then install an approved Windows SketchUp/render executor |
