...
| Production infrastructure | `[##########] 100%` | Cloudflare Pages/R2 are verified; PS.kz VPS incident is isolated, Google Cloud test VPS is online, authenticated Cloudflare -> VPS read-only smoke passes, and controlled live HTML deploy succeeded | Use the Google test VPS for temporary landing tests; rebuild/replace PS.kz before any production reuse |
...
| Google VPS recovery | 2026-06-24 | Google Cloud Debian 12 test VPS `34.140.181.91` installed with nginx, ufw, Node 20, `furniture-vps-control`, Cloudflare secrets, authenticated read-only smoke, and controlled live HTML deploy | Use `control.34-140-181-91.nip.io` as the active test VPS control origin; do not reuse compromised PS.kz without rebuild |
