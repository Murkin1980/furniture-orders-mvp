...
export async function onRequestGet(context) {
  ...
  return respond(await listCommunicationDrafts({ db: context.env.DB, orderId: url.searchParams.get("orderId") }));
}
...
export async function onRequestPost(context) {
  ...
  const result = payload.action === "review"
    ? await reviewCommunicationDraft({ db: context.env.DB, ...payload })
    : await createCommunicationDraft({ db: context.env.DB, ...payload, source: "manager", createdBy: "manager" });
  return respond(result);
}

export async function onRequestOptions() { return new Response(null, { status: 204, headers: corsHeaders() }); }
export async function onRequest() { return json({ success: false, error: "method_not_allowed", message: "Use GET or POST /api/communication-drafts." }, 405); }

function requireAdmin(context) {
  const expected = context.env.ADMIN_TOKEN;
  const authorization = context.request.headers.get("Authorization") || "";
  const token = context.request.headers.get("X-Admin-Token") || (authorization.startsWith("Bearer ") ? authorization.slice(7) : "");
