...
import { publishProposalVersion } from "../../../../src/proposals/proposal-store.js";
...
export async function onRequestPost(context) {
  ...
  const result = await publishProposalVersion({ db: context.env.DB, proposalId: context.params.id, versionNumber: body.versionNumber });
  return json(result.body, result.status);
}
export async function onRequestOptions() { return new Response(null, { status: 204, headers: headers() }); }
export async function onRequest() { return json({ success: false, error: "method_not_allowed", message: "Use POST /api/proposals/:id/publish." }, 405); }
